const CLAUDE_CODE_VERSION_PATTERN = /\bv?(\d+\.\d+(?:\.\d+)?(?:-[0-9A-Za-z.-]+)?)\b/;
...
function normalizeClaudeCodeVersion(version: string): string {
...
function parseSemver(version: string): ParsedSemver | null {
  const normalized = normalizeClaudeCodeVersion(version);
...
export function compareClaudeCodeCliVersions(left: string, right: string): number {
...
export function parseClaudeCodeCliVersion(output: string): string | null {
  ...
  return normalizeClaudeCodeVersion(match[1]);
}

export function isClaudeCodeCliVersionSupported(version: string): boolean {
  return compareClaudeCodeCliVersions(version, MINIMUM_CLAUDE_CODE_CLI_VERSION) >= 0;
}
